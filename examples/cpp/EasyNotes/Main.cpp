...
#include "Util.h"
#include "ui/MainScreen.h"			// Main UI screen
...
public:
	...
	NativeUIMoblet()
	{
		// Begin reading the stored notes and settings.
		mDataManager = new RepoManager();
		// Create the main user interface screen.
		mMainScreen = new MainScreen();
		mMainScreen->populateNotesList(mDataManager->getNotes());
//		mMainScreen->addNotesListener(mDataManager);
...
//				mDataManager->writeNotes(mMainScreen->getNotes());
				closeEvent();
			}
	    }
	}
...
private:
	...
	RepoManager* mDataManager;
	//static MAUtil::Vector<NoteGroup> mNotes;
};
